...
#-------------------------Ejercicio 2------------------------------------
df <- readxl::read_excel("data_lab5.xlsx")
#Hora Creación
...
dia_ocupado <- df_final %>%
  ...
  summarise(total = n()) %>%
  arrange(-total)
#El día lunes es el más ocupado
...
estacionalidad <- ggplot(llamadas, aes(x=mes, y=total, group=1)) +
  ...
  geom_point()
estacionalidad
#vemos que el período entre los meses de Marzo y Julio existe una estacionalidad.
#Se puede decir que es entre primavera y verano.
...
promedio_llamadas
#La llamada dura en promedio 14 minutos
...
#-------------------------Ejercicio 3------------------------------------
zodiac <- function(){
  print("Porfavor, la fecha que naciste. Empezando con el año, seguido por el mes y el día. Por ejemplo
...
flights$dep_time <- hms::parse_hm(format(strptime(sprintf('%04d', flights$dep_time), format='%H%M'), '%H:%M'))
flights$arr_time <- hms::parse_hm(format(strptime(sprintf('%04d', flights$arr_time), format='%H%M'), '%H:%M'))
flights$sched_dep_time <- hms::parse_hm(format(strptime(sprintf('%04d', flights$sched_dep_time), format='%H%M'), '%H:%M'))
flights$sched_arr_time<- hms::parse_hm(format(strptime(sprintf('%04d', flights$sched_arr_time), format='%H%M'), '%H:%M'))
flights_final <- flights

flights_final$dep_time_FH <- make_datetime(year = flights_final$year, month = flights_final$month, day = flights_final$day,
                                        hour = hour(flights$dep_time), min = minute(flights$dep_time), sec = second(flights$dep_time))
flights_final$arr_time_FH <- make_datetime(year = flights_final$year, month = flights_final$month, day = flights_final$day,
                                        hour = hour(flights$arr_time), min = minute(flights$arr_time), sec = second(flights$arr_time))
flights_final$sched_dep_time_FH <- make_datetime(year = flights_final$year, month = flights_final$month, day = flights_final$day,
                                              hour = hour(flights$sched_dep_time), min = minute(flights$sched_dep_time), sec = second(flights$sched_dep_time))
flights_final$sched_arr_time_FH <- make_datetime(year = flights_final$year, month = flights_final$month, day = flights_final$day,
                                              hour = hour(flights$sched_arr_time), min = minute(flights$sched_arr_time), sec = second(flights$sched_arr_time))
